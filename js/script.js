...
function hideResponsiveNav() {
	...
	jQuery('.nav-inner').animate({
		...
		jQuery('.scroll-down-message').css({
			'z-index': '101'
		});
	});
	jQuery('.open-nav span.hamburger').toggleClass('is-clicked');
	jQuery("#main-nav").delay(100).slideUp(700, 'easeInOutExpo', function () {
		jQuery('.nav-inner').fadeIn(200);
		jQuery('#main-nav').removeClass("nav-visible");
	});
...
(function ($) {
	...
	jQuery(window).load(function ($) {
			...
			jQuery("#page-loader .page-loader-inner").fadeOut(1000, function () {
				if (openSection) {
					jQuery('html,body').animate({
						scrollTop: jQuery("#" + openSection).offset().top - jQuery("header").height() + 80
					}, 10, 'easeInOutExpo');
				}
			});
			jQuery("#page-loader").delay(1300).animate({
				top: borderWidthHeight + 'px',
				height: jQuery(window).height() - (borderWidthHeight * 2) + 'px'
			}, 10).slideUp(1000, 'easeInOutExpo', function () {
		...
		// If no page loader
		if (jQuery('#page-loader').length < 1) {
			jQuery("body").addClass("loading-end");
		}
		...
		jQuery('header').on("click", ".open-nav", function () {
			...
			if (hidden == 'block') {
				...
				if (fullheight < 400) {
					jQuery("nav#main-nav .nav-inner").css({
	...
	var storyRightHeight = jQuery('.story-right').height();
	$('.story-left').css('height', storyRightHeight);
	console.log(splitMiniContentHeight)
